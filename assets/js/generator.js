...
$("#start").click(function() {
    ...
    async function check() {
        for (i = 0; i < ccs.length; i++) {
            ...
            if (useGate2) {
                postData.charge = true;
            }

            $.post("https://api.chkr.cc/", postData, function(response) {
...
function makeCC() {
    ...
    return template.replace(/x/gi, () => rand(0, 9).toString());
}
...
function generateCCV2() {
    ...
    return $("#cvv").val() ? "|" + $("#cvv").val() : "|" + Array.from({ length }, () => rand(0, 9)).join('');
}
